...
const MERCADOPAGO_WEBHOOK_SECRET = (0, params_1.defineSecret)("MERCADOPAGO_WEBHOOK_SECRET");
...
const MERCADOPAGO_ENVIRONMENT = (0, params_1.defineSecret)("MERCADOPAGO_ENVIRONMENT");
...
function isAdmin(context) {
    var _a, _b;
    return ((_a = context.auth) === null || _a === void 0 ? void 0 : _a.token.email_verified) === true &&
        ADMIN_EMAILS.has(normalizedEmail((_b = context.auth) === null || _b === void 0 ? void 0 : _b.token.email));
}
function requireAuthenticatedEmail(context) {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Faça login para continuar.");
    }
    if (context.auth.token.email_verified !== true) {
        throw new functions.https.HttpsError("failed-precondition", "Confirme seu e-mail antes de acessar sua licença.");
    }
...
function readGoogleIdToken(data) {
    const idToken = typeof (data === null || data === void 0 ? void 0 : data.idToken) === "string"
        ? data.idToken.trim()
        : "";
 ...
 */
exports.exchangeGoogleIdToken = functions.https.onCall(async (data) => {
    var _a, _b;
    ...
    try {
        ...
        throw new functions.https.HttpsError("unauthenticated", "Não foi possível validar sua conta Google. Tente entrar novamente.");
    }
    const googleIdentity = (_b = (_a = decoded.firebase) === null || _a === void 0 ? void 0 : _a.identities) === null || _b === void 0 ? void 0 : _b["google.com"];
    if (decoded.email_verified !== true || !normalizedEmail(decoded.email) || !(googleIdentity === null || googleIdentity === void 0 ? void 0 : googleIdentity.length)) {
        throw new functions.https.HttpsError("failed-precondition", "A conta escolhida não pôde ser confirmada como uma conta Google válida.");
    }
 ...
 */
exports.createGoogleLinkToken = functions.https.onCall(async (_data, context) => {
    ...
    if (!hasPassword) {
        throw new functions.https.HttpsError("failed-precondition", "Entre com e-mail e senha antes de conectar uma conta Google.");
    }
    return { customToken: await admin.auth().createCustomToken(user.uid) };
});
function readSecret(name, parameter) {
    try {
        return process.env[name] || (parameter === null || parameter === void 0 ? void 0 : parameter.value()) || "";
    }
    catch (_a) {
...
function manualPremiumOverride(user) {
    if (typeof (user === null || user === void 0 ? void 0 : user.manualPremiumOverride) === "boolean") {
        return user.manualPremiumOverride;
    }
    // Migração dos acessos manuais criados antes do campo explícito.
    if ((user === null || user === void 0 ? void 0 : user.subscriptionStatus) === "manual" && (user === null || user === void 0 ? void 0 : user.isPremium) === true)
        return true;
    if ((user === null || user === void 0 ? void 0 : user.subscriptionStatus) === "revoked")
        return false;
    return null;
}
function needsManualOverrideMigration(user) {
    return typeof (user === null || user === void 0 ? void 0 : user.manualPremiumOverride) !== "boolean" &&
        ((user === null || user === void 0 ? void 0 : user.subscriptionStatus) === "manual" || (user === null || user === void 0 ? void 0 : user.subscriptionStatus) === "revoked");
}
function accessFromSources(user, license, hasManualInvitation) {
    ...
    return {
        isPremium: (license === null || license === void 0 ? void 0 : license.isPremium) === true,
 ...
 */
async function claimPremiumAccess(context) {
    ...
    await db.collection("users").doc(uid).set(Object.assign(Object.assign({ email, displayName: context.auth.token.name || null, photoURL: context.auth.token.picture || null, licenseEmailVerified: true, isPremium: access.isPremium, subscriptionStatus: access.subscriptionStatus }, (needsManualOverrideMigration(userSnapshot.data())
        ? { manualPremiumOverride: manualPremiumOverride(userSnapshot.data()) }
        : {})), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
    return access;
}
exports.claimPremiumLicense = functions.https.onCall(async (_data, context) => {
...
exports.claimKiwifyLicense = functions.https.onCall(async (_data, context) => {
    return claimPremiumAccess(context);
});
/**
 ...
 */
exports.acceptLegalDocuments = functions.https.onCall(async (data, context) => {
    const email = requireAuthenticatedEmail(context);
    const uid = context.auth.uid;
    const termsVersion = typeof (data === null || data === void 0 ? void 0 : data.termsVersion) === "string" ? data.termsVersion : "";
    const privacyVersion = typeof (data === null || data === void 0 ? void 0 : data.privacyVersion) === "string" ? data.privacyVersion : "";
    if (termsVersion !== LEGAL_DOCUMENTS_VERSION || privacyVersion !== LEGAL_DOCUMENTS_VERSION) {
        throw new functions.https.HttpsError("failed-precondition", "Atualize o aplicativo para revisar e aceitar os documentos vigentes.");
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        const previousAcceptance = (_a = userSnapshot.data()) === null || _a === void 0 ? void 0 : _a.legalAcceptance;
        const alreadyAccepted = (previousAcceptance === null || previousAcceptance === void 0 ? void 0 : previousAcceptance.termsVersion) === LEGAL_DOCUMENTS_VERSION &&
            (previousAcceptance === null || previousAcceptance === void 0 ? void 0 : previousAcceptance.privacyVersion) === LEGAL_DOCUMENTS_VERSION;
        if (!alreadyAccepted) {
...
exports.setPremiumAccess = functions.https.onCall(async (data, context) => {
    if (!isAdmin(context)) {
...
exports.resetPremiumDevice = functions.https.onCall(async (data, context) => {
    if (!isAdmin(context)) {
...
exports.addPremiumEmail = functions.https.onCall(async (data, context) => {
    if (!isAdmin(context)) {
        throw new functions.https.HttpsError("permission-denied", "Apenas administradores podem alterar convites Premium.");
    }
    const email = validEmail(data === null || data === void 0 ? void 0 : data.email);
...
exports.recordOptimization = functions.https.onCall(async (data, context) => {
    requireAuthenticatedEmail(context);
    const count = Number(data === null || data === void 0 ? void 0 : data.count);
...
// ============================================================================
const MERCADOPAGO_SUBSCRIPTION_NOTIFICATION_URL = "https://us-central1-exifrankapp.cloudfunctions.net/mercadoPagoWebhook?source_news=webhooks";
...
const MERCADOPAGO_CHECKOUT_RETURN_URL = "https://us-central1-exifrankapp.cloudfunctions.net/mercadoPagoCheckoutReturn";
const CHECKOUT_CREATION_GRACE_MS = 60000;
const FINAL_PAYMENT_FAILURE_STATUSES = new Set([
    "rejected", "cancelled", "refunded", "charged_back", "charged-back"
]);
function configuredMercadoPagoPriceCents(configured, label) {
...
function mercadoPagoSubscriptionPlan(data) {
    const requestedPlan = typeof (data === null || data === void 0 ? void 0 : data.plan) === "string"
    ...
    if (requestedPlan === "monthly") {
        return {
            ...
            amountCents: configuredMercadoPagoPriceCents(readSecret("MERCADOPAGO_MONTHLY_PRICE_CENTS", MERCADOPAGO_MONTHLY_PRICE_CENTS).trim(), "mensal"),
            frequency: 1,
    ...
    if (requestedPlan === "annual") {
        return {
            ...
            amountCents: configuredMercadoPagoPriceCents(readSecret("MERCADOPAGO_ANNUAL_PRICE_CENTS", MERCADOPAGO_ANNUAL_PRICE_CENTS).trim(), "anual"),
            frequency: 12,
            frequencyType: "months"
        };
    }
...
function mercadoPagoIsSandbox(accessToken) {
...
function mercadoPagoPayerEmail(isSandbox, licenseEmail) {
    if (!isSandbox)
        return licenseEmail;
...
function mercadoPagoSubscriptionEndDate() {
...
function mercadoPagoCheckoutContext(accessToken, planCode) {
    ...
    return (0, crypto_1.createHash)("sha256").update(environment + ":" + planCode + ":" + accessToken).digest("hex");
}
function mercadoPagoWebhookSecret() {
    const secret = readSecret("MERCADOPAGO_WEBHOOK_SECRET", MERCADOPAGO_WEBHOOK_SECRET).trim();
    if (!secret)
        throw new Error("Assinatura do webhook do Mercado Pago não configurada.");
    return secret;
}
function mercadoPagoHeaders(accessToken, contentType = false) {
    ...
    if (mercadoPagoIsSandbox(accessToken))
...
function isFinalPaymentFailure(paymentStatus) {
    return typeof paymentStatus === "string" &&
        FINAL_PAYMENT_FAILURE_STATUSES.has(paymentStatus.trim().toLowerCase());
}
function mercadoPagoErrorSummary(error) {
    var _a, _b, _c;
    const payload = ((_a = error === null || error === void 0 ? void 0 : error.response) === null || _a === void 0 ? void 0 : _a.data) || {};
    const firstCause = Array.isArray(payload === null || payload === void 0 ? void 0 : payload.cause) ? payload.cause[0] : null;
    const responseHeaders = ((_b = error === null || error === void 0 ? void 0 : error.response) === null || _b === void 0 ? void 0 : _b.headers) || {};
    return {
        httpStatus: Number.isInteger((_c = error === null || error === void 0 ? void 0 : error.response) === null || _c === void 0 ? void 0 : _c.status) ? error.response.status : null,
        error: typeof (payload === null || payload === void 0 ? void 0 : payload.error) === "string" ? payload.error : null,
        message: typeof (payload === null || payload === void 0 ? void 0 : payload.message) === "string" ? payload.message : null,
        causeCode: typeof (firstCause === null || firstCause === void 0 ? void 0 : firstCause.code) === "string" ? firstCause.code : null,
        causeDescription: typeof (firstCause === null || firstCause === void 0 ? void 0 : firstCause.description) === "string" ? firstCause.description : null,
        requestId: typeof responseHeaders["x-request-id"] === "string"
            ? responseHeaders["x-request-id"]
            : null,
        networkMessage: !(error === null || error === void 0 ? void 0 : error.response) && typeof (error === null || error === void 0 ? void 0 : error.message) === "string" ? error.message : null
    };
}
async function mercadoPagoTokenAccountSummary(accessToken) {
    try {
        const response = await axios_1.default.get("https://api.mercadolibre.com/users/me", { headers: mercadoPagoHeaders(accessToken), timeout: 10000 });
        ...
        return Object.assign(Object.assign({}, mercadoPagoErrorSummary(error)), { userId: null, siteId: null });
    }
}
async function reserveMercadoPagoCheckout(uid, email, plan, checkoutContext) {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        if (existingOrderId && (existingStatus === "active" || existingStatus === "payment-failed")) {
            throw new functions.https.HttpsError("failed-precondition", existingStatus === "active"
        ...
        if (existingOrderId && existingStatus === "ready" && existingCheckoutUrl) {
            throw new functions.https.HttpsError("failed-precondition", "Existe um checkout pendente para outro plano. Conclua ou cancele essa assinatura no Mercado Pago antes de trocar de plano.");
        }
        if (existingOrderId && existingStatus === "creating") {
            if (!sameCheckoutContext || existingPlanCode !== plan.code) {
                throw new functions.https.HttpsError("aborted", "Estamos preparando outro checkout. Aguarde alguns segundos antes de trocar de plano.");
            }
            const ageMs = Date.now() - timestampMillis(session.updatedAt || session.createdAt);
            if (ageMs >= 0 && ageMs < CHECKOUT_CREATION_GRACE_MS) {
                throw new functions.https.HttpsError("aborted", "Estamos preparando seu checkout. Aguarde alguns segundos e tente novamente.");
            }
...
async function updateMercadoPagoCheckoutSession(reservation, data) {
    await db.runTransaction(async (transaction) => {
        ...
        if (session.orderId !== reservation.orderRef.id)
            return;
        transaction.set(reservation.sessionRef, Object.assign(Object.assign({}, data), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
    });
}
function resourceIdFromRequest(req) {
...
function signedHeaderParts(xSignature) {
    const parts = new Map();
...
function validMercadoPagoWebhook(req, resourceId, secret) {
    ...
    return (0, crypto_1.timingSafeEqual)(Buffer.from(expected, "hex"), Buffer.from(signed.signature, "hex"));
}
function subscriptionOrderIdFromExternalReference(reference) {
...
async function fetchMercadoPagoSubscription(subscriptionId) {
    var _a, _b, _c, _d;
    const accessToken = mercadoPagoAccessToken();
    const response = await axios_1.default.get("https://api.mercadopago.com/preapproval/" + encodeURIComponent(subscriptionId), { headers: mercadoPagoHeaders(accessToken), timeout: 10000 });
    const subscription = response.data || {};
    const id = String(subscription.id || "").trim().toLowerCase();
    const amountCents = centsFromAmount((_a = subscription.auto_recurring) === null || _a === void 0 ? void 0 : _a.transaction_amount);
    const status = String(subscription.status || "").trim().toLowerCase();
    const currency = String(((_b = subscription.auto_recurring) === null || _b === void 0 ? void 0 : _b.currency_id) || "").trim().toUpperCase();
    const frequency = Number((_c = subscription.auto_recurring) === null || _c === void 0 ? void 0 : _c.frequency);
    const frequencyType = String(((_d = subscription.auto_recurring) === null || _d === void 0 ? void 0 : _d.frequency_type) || "").trim().toLowerCase();
    const externalReference = String(subscription.external_reference || "").trim();
    if (id !== subscriptionId || amountCents === null || !status || !currency || !externalReference ||
...
function shouldReplacePendingMercadoPagoCheckout(data) {
    return (data === null || data === void 0 ? void 0 : data.replacePending) === true;
}
async function hasPendingMercadoPagoCheckoutForDifferentPlan(uid, checkoutContext) {
 ...
 */
async function cancelPendingMercadoPagoCheckoutForPlanChange(uid, email, accessToken) {
    var _a, _b;
    const sessionRef = db.collection("mercadopago_checkout_sessions").doc(uid);
    ...
    const hasCheckoutUrl = typeof session.checkoutUrl === "string" && session.checkoutUrl.startsWith("https://");
    if (!orderId || sessionStatus !== "ready" || !hasCheckoutUrl)
        return false;
    ...
    if (order.uid !== uid || validEmail(order.email) !== email) {
        throw new functions.https.HttpsError("permission-denied", "Não foi possível alterar um checkout vinculado a outra conta.");
    }
    ...
    const subscriptionId = typeof rawSubscriptionId === "string"
        ...
        : "";
    if (!/^[a-z0-9_-]{1,128}$/.test(subscriptionId))
        return false;
    ...
    if (subscription.externalReference !== "exifrank-mp-sub:" + orderId) {
        throw new functions.https.HttpsError("permission-denied", "Não foi possível confirmar a propriedade do checkout pendente.");
    }
    ...
    if (remoteStatus === "pending") {
        try {
            ...
            const response = await axios_1.default.put("https://api.mercadopago.com/preapproval/" + encodeURIComponent(subscriptionId), { status: "cancelled" }, { headers: mercadoPagoHeaders(accessToken, true), timeout: 10000 });
            const responseId = String(((_a = response.data) === null || _a === void 0 ? void 0 : _a.id) || "").trim().toLowerCase();
            remoteStatus = String(((_b = response.data) === null || _b === void 0 ? void 0 : _b.status) || "").trim().toLowerCase();
            if (responseId !== subscriptionId ||
            ...
            throw new functions.https.HttpsError("unavailable", "Não foi possível encerrar o checkout anterior. Aguarde alguns segundos e tente novamente.");
        }
    }
...
async function fetchMercadoPagoAuthorizedPayment(invoiceId) {
    var _a;
    const accessToken = mercadoPagoAccessToken();
    const response = await axios_1.default.get("https://api.mercadopago.com/authorized_payments/" + encodeURIComponent(invoiceId), { headers: mercadoPagoHeaders(accessToken), timeout: 10000 });
    ...
    return {
        ...
        subscriptionId,
        status,
        paymentStatus: typeof ((_a = invoice.payment) === null || _a === void 0 ? void 0 : _a.status) === "string" ? invoice.payment.status.toLowerCase() : null,
...
exports.createMercadoPagoSubscription = functions
    .runWith({
    secrets: [
    ...
    if (mustReplacePendingCheckout) {
        const wasCancelled = await cancelPendingMercadoPagoCheckoutForPlanChange(context.auth.uid, email, accessToken);
        if (!wasCancelled) {
            throw new functions.https.HttpsError("failed-precondition", "O checkout anterior já não está pendente. Atualize sua conta e tente novamente.");
        }
    }
    const reservation = await reserveMercadoPagoCheckout(context.auth.uid, email, plan, checkoutContext);
    if (reservation.checkoutUrl)
        return { checkoutUrl: reservation.checkoutUrl };
    ...
    try {
        const response = await axios_1.default.post("https://api.mercadopago.com/preapproval", subscriptionPayload, {
            headers: Object.assign(Object.assign({}, mercadoPagoHeaders(accessToken, true)), { "X-Idempotency-Key": reservation.orderRef.id }),
            timeout: 10000
        });
        const checkoutUrl = String(((_a = response.data) === null || _a === void 0 ? void 0 : _a.init_point) || "").trim();
        const subscriptionId = String(((_b = response.data) === null || _b === void 0 ? void 0 : _b.id) || "").trim().toLowerCase();
        ...
        await reservation.orderRef.set({
            subscriptionId,
            checkoutUrl,
            status: String(((_c = response.data) === null || _c === void 0 ? void 0 : _c.status) || "pending").toLowerCase(),
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
        }, { merge: true });
...
async function applyVerifiedMercadoPagoSubscription(subscription) {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!orderSnapshot.exists)
            throw new Error("Assinatura Mercado Pago não encontrada.");
        const order = orderSnapshot.data() || {};
        ...
        if (Number.isSafeInteger(expectedFrequency) && expectedFrequency > 0 &&
        ...
        const priorMercadoPagoPremium = existingSubscriptions.docs.some((item) => item.id !== orderId && item.data().isPremium === true);
        ...
        transaction.set(db.collection("licenses").doc(email), Object.assign(Object.assign({ email, isPremium: effectivePremium, subscriptionStatus: effectivePremium ? "subscription-active" : subscription.status }, (typeof order.planCode === "string" ? { subscriptionPlan: order.planCode } : {})), { lastOrderId: subscription.id, webhookSource: "mercadopago-subscription-api", updatedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
        if (sessionRef && ((_a = sessionSnapshot === null || sessionSnapshot === void 0 ? void 0 : sessionSnapshot.data()) === null || _a === void 0 ? void 0 : _a.orderId) === orderId) {
    ...
    if (licenseEmail)
        await syncUsersForEmail(licenseEmail);
    return true;
}
async function recordMercadoPagoInvoice(invoice) {
...
exports.mercadoPagoCheckoutReturn = functions.https.onRequest((_req, res) => {
...
function notificationTopic(req) {
    var _a;
    const bodyTopic = (_a = req.body) === null || _a === void 0 ? void 0 : _a.type;
...
exports.mercadoPagoWebhook = functions
    ...
    try {
        ...
        console.error("Erro ao validar webhook Mercado Pago:", ((_a = error === null || error === void 0 ? void 0 : error.response) === null || _a === void 0 ? void 0 : _a.status) || (error === null || error === void 0 ? void 0 : error.message) || error);
